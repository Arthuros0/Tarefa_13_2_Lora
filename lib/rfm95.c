...
bool rfm95_init(){
    ...
    // Configurar Modem (BW=125kHz, CR=4/5, SF=12)
    rfm95_write_register(REG_MODEM_CONFIG, BANDWIDTH_125K | ERROR_CODING_4_5 | EXPLICIT_MODE); 
    rfm95_write_register(REG_MODEM_CONFIG2, SPREADING_12 | CRC_ON); 
...
void rfm95_set_power(uint power) {
    if (power > 17 ) {
        power = 17; // Limita o máximo a 17 dBm
    } else if (power < 2) {
        power = 2; // Limita o mínimo a 2 dBm
    }

    // Configura o ganho de potência
    rfm95_write_register(REG_PA_CONFIG, 0x80 | (power - 2)); // 0x80 ativa o PA em modo normal
}
